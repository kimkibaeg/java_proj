...
public class Student {
	// 클래스 : 객체를 만들기 위한 틀, 객체 (인스턴스) : 클래스를 기반으로 메모리에 로드 된 것
	...
	// 2. 생성자 : 클래스 기반으로 객체 생성 시 생성자 호출
		// 1-2 : 기본 생성자 : 다른 생성자가 없을 때는 생략되어 외부에서 호출이 가능
	Student() {}
	...
	// toString() : Object 클래스의 메소드, 객체 자체를 출력 시 Heap 의 주소가 출력되지 않고 필드의 객체에 들어갈 값을 출력한다. 
