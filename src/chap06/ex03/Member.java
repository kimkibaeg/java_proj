...
public class Member {		// 회원 정보를 담은 클래스
	...
	/*
	 	접근 제어자 : OOP 의 캡슐화, 필드나 메소드 클래스에 보안 기능
	 			- 클래스 (이름 앞에) : public, default
	 			- 필드 (이름 앞에) : public, protected, default(생략), private
	 			- 메소드 (이름 앞에) : public, protected, default(생략), private
	 			- 생성자 : 
	 		public		: 다른 패키지에서 접근이 가능
	 		protected	: 다른 패키지에서 접근이 가능한데 (상속 관계가 적용 되었을 때 가능)
	...
	private int year ;	// 태어난 년	: 
	private int month ;	// 태어난 월	: 1 ~ 12
	private int day ; 	// 태어난 일	: 1 ~ 31
	private String phone ;	// 핸드폰 정보
	private String addr ;	// 주소 정보
	...
	// toString() 메소드 Overriding : 
		...
		// Overloading : 메소드 이름은 동일하고, 인풋 매개변수에 갯수나 타입에 따라서 식별하는 것.
